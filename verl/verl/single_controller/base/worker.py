...
class Worker(WorkerHelper):
    ...
    def _setup_env_cuda_visible_devices(self):
        ...
        if is_ray_noset_visible_devices:
            ...
            # Map physical GPU id → logical ordinal via CUDA_VISIBLE_DEVICES
            # position so non-zero-starting CVD (e.g. "4,5,6,7") works.
            cvd = os.environ.get("CUDA_VISIBLE_DEVICES", "")
            try:
                local_rank = str(cvd.split(",").index(str(physical_id)))
            except ValueError:
                local_rank = str(physical_id)
