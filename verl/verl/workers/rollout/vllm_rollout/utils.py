...
class vLLMColocateWorkerExtension:
    ...
    def _update_weights(self, weights: list[tuple[str, torch.Tensor]], peft_config: dict, base_sync_done: bool):
        if peft_config and base_sync_done:
            # ALAR latent stack: the LoRA adapter has `modules_to_save=["projector"]`
            # because our custom projector module is trained fully (not LoRA-wrapped).
            # vLLM's PEFTHelper rejects non-None modules_to_save, and vLLM's plain
            # Qwen2 doesn't have a projector module anyway — its weights flow to vLLM
            # via the separate LATENT_SYNC_PATH file IPC. Strip both here:
            #   1. peft_config.modules_to_save -> None (lets vLLM accept it)
            #   2. projector/modules_to_save tensors -> drop (vLLM can't load them)
            if peft_config.get("modules_to_save"):
                peft_config = {**peft_config, "modules_to_save": None}
            weights = {
                n: w for n, w in weights
                if "projector" not in n and ".modules_to_save." not in n
            }
            ...
            if is_fp8_model(self.model_runner.vllm_config):
                ...
                # V9: filter reasoning_net.* — vLLM's plain Qwen2 can't accept them.
                weights = ((n, w) for n, w in weights if not n.startswith("reasoning_net."))
