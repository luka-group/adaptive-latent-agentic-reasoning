...
# Latent-reasoning integration: register custom HF model_types BEFORE any
# HFModelConfig instantiation fires AutoConfig.from_pretrained. No-op for
# vanilla verl use (alar.modeling absent from PYTHONPATH).
#
# Baselines export ALAR_BASELINE=1 to skip even the AutoConfig registration,
# so vanilla Qwen2 models aren't shadowed by our LatentQwen2Config dispatch.
...
if not _alar_os.environ.get("ALAR_BASELINE"):
    try:
        ...
        pass
# NOTE: RL update_actor sync hook is installed at the bottom of
# verl/workers/fsdp_workers.py (after ActorRolloutRefWorker is fully defined).
# Installing here would circular-import.
