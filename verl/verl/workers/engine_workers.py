...
# Latent-reasoning integration: install alar.rl patches at module load so
# Ray worker processes that import this module (the new engine-backed
# worker path) get the projector sync + LoRA-adapter save hooks installed
# before any actor instance is constructed. Mirrors the hook at the
# bottom of verl/workers/fsdp_workers.py. No-op if alar.rl.install_patches
# isn't on PYTHONPATH (vanilla verl unaffected).
#
# Baselines export ALAR_BASELINE=1 to skip patches entirely.
import os as _alar_os
if not _alar_os.environ.get("ALAR_BASELINE"):
    try:
        from alar.rl.install_patches import install as _alar_install_patches
        _alar_install_patches()
    except ImportError:
        pass
    except Exception as _alar_install_e:
        print(f"[latent_install] engine_workers hook failed: {_alar_install_e}", flush=True)
