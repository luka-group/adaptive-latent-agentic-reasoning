...
# V11 Stage 0 rollout-saving hook (opt-in via env var). When STAGE0_REWARD1_LOG
# is set, rollouts with score >= STAGE0_REWARD1_THRESHOLD are appended to a
# per-rank JSONL — captured (prompt, response, ground_truth) tuples become the
# V11 Stage 1 SFT corpus. Inactive (no overhead, zero behavioral diff) when
# the env var is unset, so search-domain RL training is unaffected.
...
class NaiveRewardManager(AbstractRewardManager):
    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        for i in range(len(data)):
            ...
            if (
                _STAGE0_LOG_PATH
                and data_source == _STAGE0_DATA_SOURCE
                ...
                try:
                    ...
                    # Defensive ID: RANK may be unset under Ray. PID guarantees
                    # uniqueness whether reward runs on driver or in workers.
