...
# V9 integration: import modeling_v9 if present on PYTHONPATH so that HF
# AutoConfig recognises model_type=qwen2_reasoning when verl workers load our
# custom checkpoints. No-op if the module isn't importable (vanilla verl use).
try:
    import modeling_v9  # noqa: F401
except ImportError:
    pass
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode(self):
        ...
        # V9: filter out reasoning_net.* keys before shipping to vLLM — those
        # live on our Qwen2ForCausalLMWithReasoning actor but not on vLLM's
        # plain Qwen2ForCausalLM. The reasoning_net sync to the rollout HF
        # holder is handled separately via LATENT_RAG_V9_SYNC_PATH.
        _items = ((n, p) for n, p in _items if not n.startswith("reasoning_net."))
        per_tensor_param = (
            (
                name,
...
# ALAR RL — install FSDP→file sync hook on ActorRolloutRefWorker.update_actor.
# Fires at module-load time so it propagates to every Ray worker process
# before any worker instance is constructed. Idempotent; no-op if alar.rl
# absent from PYTHONPATH (vanilla verl unaffected).
#
# Baselines export ALAR_BASELINE=1 to skip the patches entirely, so the
# text-CoT comparison runs on unmodified verl semantics.
import os as _os
if not _os.environ.get("ALAR_BASELINE"):
    try:
        ...
        print(f"[latent_install] failed: {_e}", flush=True)
